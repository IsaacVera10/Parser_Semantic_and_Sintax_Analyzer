...
Token* Scanner::nextToken() {
  ...
  if (c == '%') {
    while (c != '\n' && c != '\0') c = nextChar();
    // Después de encontrar un comentario, llama recursivamente a nextToken
    return nextToken();
  }
...
Instruction* Parser::parseInstruction() {
  ...
  if (match(Token::POP) || match(Token::ADD) || match(Token::SUB) || match(Token::MUL) || match(Token::DIV) || match(Token::DUP) || match(Token::SWAP) || match(Token::PRINT)) {
    tipo = 0;
    ttype = previous->type;
  ...
  if (tipo == 0) {
    ...
    // Instrucción con argumento de etiqueta
    instr = new Instruction(label, Token::tokenToIType(ttype), jmplabel);
    advance();
  } else {
    // Instrucción con argumento entero
    int arg = stoi(current->lexema);
    instr = new Instruction(label, Token::tokenToIType(ttype), arg);
    advance(); // Avanza al siguiente token después del número entero
  }
